"use client";

import type { Lap, Stint } from "@/lib/openf1";
import { formatSectorTime } from "@/lib/format";

type SectorTimeTableProps = {
  laps: Lap[];
  stints: Stint[];
  driverName: string;
};

function compoundTag(compound: string) {
  const c = compound.toUpperCase();
  if (c.includes("SOFT")) return { text: "S", cls: "tyre-soft" };
  if (c.includes("MEDIUM")) return { text: "M", cls: "tyre-medium" };
  if (c.includes("HARD")) return { text: "H", cls: "tyre-hard" };
  if (c.includes("INTER")) return { text: "I", cls: "tyre-inter" };
  if (c.includes("WET")) return { text: "W", cls: "tyre-wet" };
  return { text: "?", cls: "tyre-hard" };
}

function getCompoundForLap(lapNumber: number, stints: Stint[]): string | null {
  for (const stint of stints) {
    if (lapNumber >= stint.lap_start && lapNumber <= stint.lap_end) return stint.compound;
  }
  return null;
}

const COL_GREEN = "#4ae38f";
const COL_DEFAULT = "rgba(255,255,255,0.45)";

export function SectorTimeTable({ laps, stints, driverName }: SectorTimeTableProps) {
  const validLaps = laps.filter((l) => !l.is_pit_out_lap);

  const s1Values = validLaps.map((l) => l.duration_sector_1).filter((v): v is number => v != null);
  const s2Values = validLaps.map((l) => l.duration_sector_2).filter((v): v is number => v != null);
  const s3Values = validLaps.map((l) => l.duration_sector_3).filter((v): v is number => v != null);

  const bestS1 = s1Values.length > 0 ? Math.min(...s1Values) : null;
  const bestS2 = s2Values.length > 0 ? Math.min(...s2Values) : null;
  const bestS3 = s3Values.length > 0 ? Math.min(...s3Values) : null;
  const bestTotal = bestS1 != null && bestS2 != null && bestS3 != null ? bestS1 + bestS2 + bestS3 : null;

  const isBest = (value: number | null, best: number | null) =>
    value != null && best != null && Math.abs(value - best) < 0.001;

  if (validLaps.length === 0) {
    return (
      <div className="panel rounded-[28px] p-5">
        <p className="telemetry-kicker text-xs text-[var(--accent-cool)]">Sector splits</p>
        <div className="mt-6 rounded-[18px] border border-dashed border-white/10 px-4 py-12 text-center text-sm text-white/35">
          No sector data available.
        </div>
      </div>
    );
  }

  return (
    <div className="panel rounded-[28px] p-5">
      <p className="telemetry-kicker text-xs text-[var(--accent-cool)]">Sector splits</p>
      <p className="mt-2 text-sm text-white/55">
        {driverName}&apos;s sector times per lap. <span style={{ color: COL_GREEN }}>Green</span> = personal best for that sector.
      </p>

      {bestTotal != null && (
        <div className="mt-3 flex items-center gap-4 rounded-[14px] border border-white/6 bg-white/3 px-4 py-2.5 text-xs">
          <span className="text-white/40">Theoretical best:</span>
          <span className="font-mono" style={{ color: COL_GREEN }}>{formatSectorTime(bestS1)}</span>
          <span className="text-white/15">+</span>
          <span className="font-mono" style={{ color: COL_GREEN }}>{formatSectorTime(bestS2)}</span>
          <span className="text-white/15">+</span>
          <span className="font-mono" style={{ color: COL_GREEN }}>{formatSectorTime(bestS3)}</span>
          <span className="text-white/15">=</span>
          <span className="font-mono font-semibold" style={{ color: COL_GREEN }}>{formatSectorTime(bestTotal)}</span>
        </div>
      )}

      <div className="mt-4 max-h-[420px] overflow-y-auto pr-1">
        <table className="w-full text-xs">
          <thead className="sticky top-0 z-10" style={{ background: "var(--surface, #0a0b10)" }}>
            <tr className="border-b border-white/8 text-[10px] uppercase tracking-widest text-white/30">
              <th className="pb-2 text-left font-normal">Lap</th>
              <th className="pb-2 w-8 text-center font-normal">Tyre</th>
              <th className="pb-2 text-right font-normal">Sector 1</th>
              <th className="pb-2 text-right font-normal">Sector 2</th>
              <th className="pb-2 text-right font-normal">Sector 3</th>
              <th className="pb-2 text-right font-normal">Lap time</th>
            </tr>
          </thead>
          <tbody>
            {validLaps.map((lap) => {
              const compound = getCompoundForLap(lap.lap_number, stints);
              const tag = compound ? compoundTag(compound) : null;
              const total =
                lap.duration_sector_1 != null && lap.duration_sector_2 != null && lap.duration_sector_3 != null
                  ? lap.duration_sector_1 + lap.duration_sector_2 + lap.duration_sector_3
                  : lap.lap_duration;

              return (
                <tr key={lap.lap_number} className="border-b border-white/5 hover:bg-white/[0.02]">
                  <td className="py-1.5 font-mono text-white/60">{lap.lap_number}</td>
                  <td className="py-1.5 text-center">
                    {tag ? <span className={`tyre ${tag.cls} inline-flex !text-[9px] !px-1.5 !py-0`}>{tag.text}</span> : <span className="text-white/20">—</span>}
                  </td>
                  <td className="py-1.5 text-right font-mono" style={{ color: isBest(lap.duration_sector_1, bestS1) ? COL_GREEN : COL_DEFAULT }}>
                    {formatSectorTime(lap.duration_sector_1)}
                  </td>
                  <td className="py-1.5 text-right font-mono" style={{ color: isBest(lap.duration_sector_2, bestS2) ? COL_GREEN : COL_DEFAULT }}>
                    {formatSectorTime(lap.duration_sector_2)}
                  </td>
                  <td className="py-1.5 text-right font-mono" style={{ color: isBest(lap.duration_sector_3, bestS3) ? COL_GREEN : COL_DEFAULT }}>
                    {formatSectorTime(lap.duration_sector_3)}
                  </td>
                  <td className="py-1.5 text-right font-mono text-white/55">
                    {formatSectorTime(total)}
                  </td>
                </tr>
              );
            })}
          </tbody>
        </table>
      </div>
    </div>
  );
}
